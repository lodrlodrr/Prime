...
// Basic security headers and CORS for Vercel
  ...
  try {
    ...
    sessionMiddleware = session({
      ...
      store: MongoStore.create({
        ...
        autoRemove: 'native',
        // Add options for better compatibility with Render
...
async function initDatabase() {
  try {
    ...
    const timeoutPromise = new Promise((resolve) => {
      setTimeout(() => {
        console.warn('[!] Database initialization timeout, continuing with limited functionality');
        resolve(null);
    ...
    if (result) {
      ...
      console.warn('[!] Database initialization timed out or failed, continuing with limited functionality');
    }
  } catch (err) {
...
async function startServer() {
  try {
    ...
    const server = app.listen(PORT, '0.0.0.0', () => {
      ...
      console.log(`[+] Discord callback URL: ${process.env.DISCORD_CALLBACK_URL || `https://primebot-s6z3.onrender.com/auth/discord/callback`}`);
      console.log(`[+] Application started with ${db.db ? 'database connected' : 'database disconnected'}`);
    });
...
// Export the app and start function for use in the unified server
