...
class Database {
  ...
  async _serverlessConnect() {
    ...
    try {
      ...
      this.db = this.client.db(process.env.MONGODB_DB_NAME || 'primebot');
      return this.db;
    } catch (err) {
  ...
  // Initialize database connection with serverless-friendly approach
  async init(maxRetries = 2) {
    let lastError;
    
    // If we're in Vercel or production, use optimized connection
    if (process.env.VERCEL || process.env.NODE_ENV === 'production') {
      if (this.db) {
        return this.db; // Reuse existing connection in serverless
      }
      try {
        return await this._serverlessConnect();
      } catch (err) {
        console.warn('[-] Serverless connection failed:', err.message);
